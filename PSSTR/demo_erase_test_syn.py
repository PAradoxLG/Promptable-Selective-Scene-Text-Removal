...
def get_args_parser():
    ...
    parser.add_argument("--output", type=str, default='./output_result_syn',
    ...
    parser.add_argument("--mask_path", type=str, default="D:\\lg\\word_mask_syn",
                        help="mask_path")
    parser.add_argument("--label_path", type=str, default="D:\\lgsyn_dataset\\label",
                        help="label path.")
...
if __name__ == '__main__':
    ...
    for path in tqdm(args.input, disable=not args.output):
        ...
        if args.promptable and args.erase_mode:
            ...
            # m_path = os.path.join(args.mask_path, "{:04}".format(int(basename)) + '.txt')
            l_path = os.path.join(args.label_path, basename + '.png')
            ...
            try:
                ...
                liney = [h-int(float(i)) for i in liney]
            except:
                continue
            ...
            #     mask=input_mask,
            #     words=input_word,
            #     num_point_per_area= 10,
            # )
            # print(pr_mask.shape, pr_iou.shape, image_hr.shape, mask_hr.shape)
            # print(pr_mask.shape)
            # print(pr_iou.shape)
            # print(image_hr.shape)
            # print(mask_hr.shape)

            # image_hr = image_hr.squeeze(0).transpose(1, 2, 0) 
            image_hr = image_hr.transpose(1, 2, 0) 
            # print(image_hr.shape) # (427, 640, 3)
            # print(type(image_hr)) # <class 'numpy.ndarray'>
            # io.imsave('output.png', image_hr)
            # print(args.output + basename)
            # io.imsave(args.output + basename + '_output.png', image_hr)
            mask_hr = mask_hr.transpose(1, 2, 0).astype(np.uint8)
            # if prompt_type == 'mask':
            #     # print(mask_hr.shape,type(mask_hr))
            #     # mask_hr = gt_mask[:,:,np.newaxis]
            #     mask_hr = find_overlapping_regions(mask_hr, input_mask[:,:,np.newaxis])
            #     if len(mask_hr)!=0:
            #         mask_hr = mask_hr[0]
            #         mask_hr = mask_hr['mask']
            #         mask_hr = mask_hr[:,:,np.newaxis].astype(np.uint8)
            #     else:
            #         mask_hr = input_mask[:,:,np.newaxis]
            ...
            # print(mask_hr.sum())
            # cv2.imwrite("omask.png",(mask_hr*255).astype(np.uint8))
            # cv2.imwrite(args.output + basename + '_ori_mask.png',(mask_hr*255).astype(np.uint8))
            # cv2.imwrite("obmask.png",(mask_blurred*255).astype(np.uint8))
            # cv2.imwrite(args.output + basename + '_mask.png',(mask_blurred*255).astype(np.uint8))
            # cv2.imwrite("onmask.png",(mask_np*255).astype(np.uint8))
            # cv2.imwrite(args.output + basename + '_mask.png',(mask_np*255).astype(np.uint8))
            # io.imsave('output_com.png', com_image)
            # io.imsave('output_blurred.png', blurred_com_image)
