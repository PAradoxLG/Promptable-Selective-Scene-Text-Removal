...
def get_args_parser():
    ...
    #                     help="label path.")
    parser.add_argument("--label_path", type=str, default="C:/Users/gran/Hi-SAM/datasets/FlickrST/test/label",
                        help="label path.")
...
if __name__ == '__main__':
    ...
    for path in tqdm(args.input, disable=not args.output):
        ...
        if args.promptable and args.erase_mode:
            ...
            all_mask = np.clip(pre_mask + all_mask, 0, 1)
            cv2.imwrite(args.output + 'mask/' + basename + '.png', (all_mask*255).astype(np.uint8))
