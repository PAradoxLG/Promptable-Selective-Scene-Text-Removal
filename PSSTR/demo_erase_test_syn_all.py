...
def get_args_parser():
    ...
    parser.add_argument("--mask_path", type=str, default="D:\\lgsyn_dataset\\mask",
                        help="mask_path")
...
if __name__ == '__main__':
    ...
    for path in tqdm(args.input, disable=not args.output):
        ...
        if args.promptable and args.erase_mode:
            ...
            h, w = image.shape[:2]
            m_path = os.path.join(args.mask_path, basename + '.png')
            ...
            fmask = Image.open(m_path).convert('L')
            fmask = fmask.resize((w, h), Image.Resampling.NEAREST)  # 注意PIL尺寸为(width, height)
            input_mask = np.array(fmask)
            ...
            image_hr = image_hr.transpose(1, 2, 0) 
            # mask_hr = input_mask.transpose(1, 2, 0).astype(np.uint8)
            mask_hr = input_mask[:,:,None].astype(np.uint8)
            mask_hr = mask_hr/255
            mask_blurred = cv2.GaussianBlur(mask_hr*255, (23, 23), 0)/255
            mask_blurred = mask_blurred[:,:,np.newaxis]
            mask_np = 1-(1-mask_hr) * (1-mask_blurred)
            ...
            io.imsave(args.output + 'image/' + basename + '.png', com_image)
        else:
