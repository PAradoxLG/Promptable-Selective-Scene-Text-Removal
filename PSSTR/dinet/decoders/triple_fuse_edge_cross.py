...
from models.interaction_mechanism.generate_inter_condition import FFTA
    ...
    def forward(self, inp, encs):
        ...
        for (
            ...
            if step < self.layer_decoders - 1: # only upsample for [layer_decoders - 1] times
                seg = reduce_seg(torch.cat([seg, enc], dim=1))
                rm = reduce_rm(torch.cat([rm, enc], dim=1))
                seg = up_seg(seg)
                rm = up_rm(rm)
