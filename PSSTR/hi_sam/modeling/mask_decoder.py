...
class UniMaskDecoder(nn.Module):
    def __init__(
        self,
        *,
        transformer_dim: int,
    ...
    def forward(
        ...
        Predict masks given image and prompt embeddings.
        """
        masks, iou_pred, word_masks, mask_embeddings = self.predict_masks(
            image_embeddings=image_embeddings,
            image_pe=image_pe,
            sparse_prompt_embeddings=sparse_prompt_embeddings,
            dense_prompt_embeddings=dense_prompt_embeddings,
        ...
        masks = masks[:, mask_slice, :, :]
        iou_pred = iou_pred[:, mask_slice]
        return masks, iou_pred, word_masks, mask_embeddings
    ...
    def predict_masks(
        ...
        for i in range(self.num_mask_tokens):
            hyper_in_list.append(self.output_hypernetworks_mlps[i](mask_tokens_out[:, i, :]))
        hyper_in = torch.stack(hyper_in_list, dim=1) # (b, 2, 32)
        b, c, h, w = upscaled_embedding.shape
        masks = (hyper_in @ upscaled_embedding.view(b, c, h * w)).view(b, -1, h, w) # (b, 2, 32) @ (b, 2, 256*256) -> (b, 2, 256*256) ->(b, 2, 256, 256)
        iou_pred = self.iou_prediction_head(iou_token_out)
        ...
        return masks, iou_pred, word_masks, src
