...
def l1_loss(prediction: torch.Tensor,
            target: torch.Tensor):
...
def loss_image(preds, gts):
    return l1_loss_jit(preds, gts)


# def pcr_loss(feat_in, feat_gt):
#     pcr_losses = []
#     for i in range(3):
#         pcr_losses.append(F.l1_loss(feat_in[i], feat_gt[i]))
#     return sum(pcr_losses)

# pcr_loss_jit = torch.jit.script(
#     pcr_loss
# )  # type: torch.jit.ScriptModule

def percetual_loss(feat_pred, feat_gt):
    pcr_losses = []
    for i in range(3):
        pcr_losses.append(F.l1_loss(feat_pred[i], feat_gt[i]))
    return sum(pcr_losses)

def gram_matrix(feat):
    (b, ch, h, w) = feat.size()
    print(feat.shape, feat.min(), feat.max())
    feat = feat.view(b, ch, h * w).to(torch.float32)
    feat_t = feat.transpose(1, 2)
    # 检查是否存在 NaN 或 Inf
    if torch.isnan(feat).any():
        raise ValueError("NaN detected in Gram matrix.")
    if torch.isinf(feat).any():
        raise ValueError("Inf detected in Gram matrix.")
    gram = (torch.bmm(feat, feat_t)) / (ch * h * w + 1e-5)
    print(ch * h * w)
    print(gram.shape,gram.min(),gram.max())

    # 检查是否存在 NaN 或 Inf
    # if torch.isnan(gram).any() or torch.isinf(gram).any():
    #     raise ValueError("NaN or Inf detected in Gram matrix.")
    if torch.isnan(gram).any():
        raise ValueError("NaN detected in Gram matrix.")
    if torch.isinf(gram).any():
        raise ValueError("Inf detected in Gram matrix.")

    return gram

def style_loss(feat_pred, feat_gt):
    style_losses = []
    for i in range(3):
        style_losses.append(F.l1_loss(gram_matrix(feat_pred[i]), gram_matrix(feat_gt[i])))
