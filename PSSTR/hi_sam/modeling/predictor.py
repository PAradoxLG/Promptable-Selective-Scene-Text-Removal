...
class SamPredictor:
    ...
    def predict(
        ...
        if promptable:
            ...
            pr_masks, pr_iou, pr_word_masks = self.predict_torch(
                multimask_output,
    ...
    def predict_torch(
        ...
        if promptable:
            ...
            # print("visual embedding >>> ", point_embeddings.shape)
            if words is not None:
                words_embedding = self.model.word_embedding(words)
                # lg20230315
                words_embedding = self.model.word_chan_proj(words_embedding)
                sparse_prompt_embeddings = words_embedding

            pr_masks, pr_iou_pred, pr_word_masks, mask_embeddings = self.model.unimask_decoder(
                image_embeddings=self.features,
                image_pe=self.model.prompt_encoder.get_dense_pe(),
                sparse_prompt_embeddings=sparse_prompt_embeddings,
                multimask_output=True
            )
            # print("Model Output >>> ", pr_masks.shape, pr_iou_pred.shape, pr_word_masks.shape)
            # Upscale the masks to the original image resolution
            pr_masks = self.model.postprocess_masks(pr_masks[:, :, :, :], self.input_size, self.original_size)
            pr_word_masks = self.model.postprocess_masks(pr_word_masks, self.input_size, self.original_size)
            if not return_logits:
                pr_masks = pr_masks > self.model.mask_threshold
                pr_word_masks = pr_word_masks > self.model.mask_threshold
            return pr_masks, pr_iou_pred, pr_word_masks
        else:
            raise RuntimeError("--promptable must be set.")
