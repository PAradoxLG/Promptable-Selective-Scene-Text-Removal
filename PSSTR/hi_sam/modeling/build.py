...
from .mask_decoder import MaskDecoder, HiDecoder, UniMaskDecoder
...
def _build_sam(
    ...
    if args.word_prompt:
        model.word_prompt = True
        model.word_embedding = WordEmbedding(
            max_len=max_word_length,
            emb_dim=768,
        )

        if args.word_embedding_weight:
            word_embedding_path = args.word_embedding_weight
            with open(word_embedding_path, "rb") as f:
                word_embedding_dict = torch.load(f, map_location="cpu")
            dict_keys = word_embedding_dict.keys()
            if 'model' in dict_keys:
                word_embedding_dict = word_embedding_dict['model']
            info = model.word_embedding.load_state_dict(word_embedding_dict, strict=False)
            print(f'word_embedding matched info: {info}')
            model.word_embedding.freeze()
        ...
        #         unimask_decoder_dict = torch.load(f, map_location="cpu")
        #     dict_keys = unimask_decoder_dict.keys()
        #     if 'optimizer' in dict_keys or 'lr_scheduler' in dict_keys or 'epoch' in dict_keys:
        #         unimask_decoder_dict = unimask_decoder_dict['model']
        #     info = model.word_embedding.load_state_dict(unimask_decoder_dict, strict=False)
        #     print(f'word_embedding matched info: {info}')

        model.word_chan_proj = nn.Linear(768, prompt_embed_dim)
        # nn.init.orthogonal_(model.word_chan_proj.weight, gain=0.06)
        # nn.init.constant_(model.word_chan_proj.bias, 0.2)
        
    if args.promptable:
        model.promptable = True
        model.unimask_decoder = UniMaskDecoder(
        ...
        if args.unimask_decoder_weight:
            ...
            if 'optimizer' in dict_keys or 'lr_scheduler' in dict_keys or 'epoch' in dict_keys:
                ...
                # lg20250305
                if 'word_chan_proj' in param_dict.keys():
                    word_chan_proj_dict = param_dict['word_chan_proj']
                if 'unimask_decoder' in param_dict.keys():
                    unimask_decoder_dict = param_dict['unimask_decoder']
                # end
            del param_dict
            info = model.unimask_decoder.load_state_dict(unimask_decoder_dict, strict=False)
            print(f'unimask_decoder matched info: {info}')
            # lg20250305
            if args.word_prompt:
                info = model.word_chan_proj.load_state_dict(word_chan_proj_dict, strict=False)
                print(f'word_chan_proj matched info: {info}')
    ...
    if checkpoint:
        ...
        for p in model.mask_decoder.parameters():
            p.requires_grad = False
        print("Freeze SAM's mask decoder")

    if args.erase_mode:
        model.erase_mode = True
        for p in model.unimask_decoder.parameters():
            p.requires_grad = False
        print("Freeze unimask_decoder.")
        
        # lg20250305
        for p in model.word_chan_proj.parameters():
            p.requires_grad = False
        print("Freeze word embedding channel projection.")
